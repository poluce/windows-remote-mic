...
>
> 实验结论（已实测）：HidHide 路线**不可用**。能隐藏设备，但打开句柄仍 AccessDenied；官方 FAQ 明确不支持键盘/鼠标类设备。返回/音量键改走 Frida Gadget 注入 WUDFHost 的 HOGP 旁路，详见 [docs/项目/问题记录-读取返回音量键.md](docs/项目/问题记录-读取返回音量键.md)。
>
> HidHide 是 **HID 设备可见性防火墙**，只控制“哪个进程能看到设备”，**不提供读取 HID 报告的 API，也不会把 HID-over-GATT（HOGP）设备从 Windows 蓝牙 HID 驱动栈中“抢走”并交给你独占**。下文保留调研记录，不再作为可行方案。
