...
- [ ] `core-hid`
  - [x] Raw Input 捕获框架（Windows 隐藏窗口 + 消息循环，待真机验证）
  - [ ] HID 路径校验，避免同型号多设备串扰
...
- [x] `core-mapping`
  ...
  - [x] 单击 / 双击 / 长按触发条件 + 按住重复（TriggerDetector）
...
- [x] Rust 单元测试：ATVV / ADPCM / 动作 / 配置（已有部分，随模块逐步补充）
- [ ] Rust 集成测试：BLE contract / 音频 contract / 按键链
- [ ] 前端测试：Vitest + Playwright
- [ ] CI 全绿门禁（当前 CI 已配置，需保持全绿）
- [ ] 真机验收清单（见 `docs/项目/真机验收.md`）
  - [ ] RC003 配对 / 重连 / 断线恢复
  - [ ] 12 个普通键单次触发、无双触发
  - [ ] 双击 / 长按 / 按住重复
  - [ ] Win+H 系统语音输入出字（基础验收）
