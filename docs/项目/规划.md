...
> 状态：核心已实现并真机验证（2026-09-01）；产品化（安装器/签名/自动更新）待推进
...
- BLE/ATVV：连接、能力查询、Audio/Control 通知、ADPCM 解码、CABLE 输出已真机跑通。
- 按键：全部 13 键经 HOGP 驱动层拦截（默认开启，设置页可切换）→ 应用映射表注入；单击/双击/长按（长按只触发一次）+ 麦克风按下/松开（长按门控 PTT）；长按阈值/双击窗口可设置；菜单键默认呼出快捷菜单。
- 快捷菜单：右下角扇形菜单，打开时进入遥控器独占输入模式，方向/确定/菜单/返回等键直接操作菜单；菜单状态持久化。
- 语音：麦克风 Press/Release → Win+H；音频走 ATVV Audio 特征，不受按键拦截影响。
- 待补：Win+H 出字端到端、双击、长时间语音、安装包等（见 `真机验收.md` / `任务清单.md`）。
...
| `core-hid` | Raw Input 捕获、HOGP 旁路/拦截、RC003 HID usage 识别、双触发抑制 |
| `core-input` | `SendInput` 按键注入、快捷键动作、打开应用、音量控制 |
| `core-mapping` | 单击/双击/长按触发条件、按下/松开 PTT、按键映射配置、热加载 |
| `core-dispatch` | 按键调度：触发检测 → 映射 → 动作执行；快捷菜单独占输入模式与应用事件出口 |
| `core-config` | 原子化 JSON 配置、迁移、热加载 |
| `core-voice` | 语音桥编排：ATVV 字节 → ADPCM → 48k 输出；模拟链路 |
| `core-log` / `core-stats` | 统一日志 / 本机统计 |
| `ui`（src/） | Tauri + React 设置界面（连接/按键映射/诊断/引导） |
...
RC003 普通按键
  ...
  → 识别物理 HID usage
  → 触发条件判定（单击/双击/长按，长按只触发一次；麦克风为按下/松开 PTT）
  → 根据用户映射执行动作（菜单键默认打开快捷菜单；快捷菜单打开时按键直转菜单）
  → SendInput 注入键盘/系统动作 或 应用事件（快捷菜单）
```
...
- 单击/双击/长按（长按只触发一次）+ 麦克风按下/松开 PTT
- 自定义快捷键、打开应用、系统音量/播放控制、快捷菜单
...
| 依赖 | 用途 |
| --- | --- |
| `windows-rs` / `windows-sys` | WinRT BLE、Win32 API、Raw Input、SendInput |
| `cpal` / WASAPI | Windows 音频输出 |
| `serde` / `serde_json` | 配置、协议数据 |
| `core-log`（自有） | 统一文件日志（分级、DEBUG 开关、自动轮转） |
...
| Vite | 前端构建 |
| @tauri-apps/api / plugin-opener | Tauri 前端 API 与系统打开 |
...
│   ├── core-hid/               # Raw Input / HOGP 旁路与拦截
│   ├── core-input/             # 按键注入/动作执行
│   ├── core-mapping/           # 按键映射/手势/触发
│   ├── core-dispatch/          # 按键调度 + 快捷菜单独占输入
│   ├── core-config/            # 配置持久化
│   ├── core-voice/             # 语音桥编排
│   ├── core-log/               # 统一日志
│   └── core-stats/             # 本机统计
├── src/                        # Tauri React 前端源码（连接/映射/诊断/引导）
│   ├── pages/
│   ├── components/
│   └── store/
├── src-tauri/                  # Tauri 后端壳
├── public/
│   └── quick-menu.html         # 快捷菜单独立页面
├── docs/
│   ├── 项目/
│   │   ├── 规划.md
│   │   ├── 任务清单.md
...
| M2 | 按键链路 | 真机 13 键（12 普通 + 麦克风 PTT）可靠触发，无双触发 |
